...
In some IRL methods, failed demonstrations have been treated as noise \cite{zheng2014robust} and filtered out in order to improve robustness. However, such methods do not actually use such demonstrations for learning.
...
In this section, we describe our progress on the topic of learning social cost functions for social conversation through feedback that is provided from 1) the visitor and 2) the interaction targets, as a follow-up on the work that has been presented in Deliverable 5.1 \cite{d5.1}. There, we focused mostly on extracting a measure of social cost associated with the facial expressions of the visitor that were more significantly correlated with negative feedback. Here, we focus on obtaining social cost functions over a space of body pose states for the robot, which can then be used to inform the body pose control policies that are being developed also within Work Package 5.
...
In total, $12$ subjects were involved in our local experiments, which consisted of a mix of student and faculty voluteers. A video of a part of one of our experiments can be found at: \url{https://staff.fnwi.uva.nl/j.v.teixeiradesousamessias/teresa_vids/local_experiments.ogv}. There, the data collected both from the camera facing the visitor and also from the onboard RGB/D camera on the robot are shown in real time. The alternating behavior of our approach scenarios is also showcased -- the visitor first approaches the robot, giving us data on the preferred range for interaction. Then, the robot approaches the person, either attempting to elicit feedback from the subjects, or by performing what the expert considers to be a ``socially acceptable'' interaction.
...
The consortium partners at UT have also conducted an experiment which focused on social positioning of the TERESA robot within group conversations. During this experiment, which is described in Deliverable 3.1 \cite{d3.1}, our partners focused on studying the proxemics in the social conversation task through the theory of F-formations \cite{kendon1990conducting} (i.e. different spatial arrangements for persons during group conversations) for situations with multiple interaction targets. Although these datasets do not contain real-time feedback from the participants, and therefore are more limited for the purposes of learning social cost functions for social conversation than those that were collected during the experiments performed locally at the UvA, they did include an \emph{a posteriori} evaluation of the quality of the interaction behaviors at the end of each episode. Our partners have granted us access to their datasets, consisting of robot trajectories and user scores collected after conversation episodes. We have used this dataset to learn social cost functions for social conversation from the \emph{a posteriori} feedback given by the interaction targets.
