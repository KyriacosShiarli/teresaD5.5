...
This deliverable has summarised our progress for Work Package 5 from month 15 of the project until month 30. Significant progress has been achieved in all aspect of social behaviour learning on the TERESA robot. 
 ...
 In terms of social conversation, there are two directions of future work we are currently pursuing and for which we plan to report in month 36. Firstly we aim for integrate these cost functions in order to learn a single social conversation policy under which the robot operates \todo{Diederik}. Secondly we aim to make the resulting policy more robust to false positives from the robots social detectors using advances filtering techniques such as partially observable Markov decision processes (POMDPS) and long-short memory networks (LSTM).
